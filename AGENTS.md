...
- `scripts/production/`: pinned production target, additive migration manifest,
  secret bootstrap, rollback priming, resumable preparation and rollout, and
...
npm run test:deployment # Exercise production preparation and rollout against disposable databases
...
`src/lib/exam.test.ts`). Production deployment tests require Podman and run in
