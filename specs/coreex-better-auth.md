# ACN FDE CoreEx backend and Better Auth account system

> **Status: implemented specification and delivery record.** The optional
> CoreEx/Better Auth stack, account mode, data controls, production Container,
> migrations, rollout/recovery tooling, and resilience coverage are implemented
> through issue #81. The checklist records the completed delivery order. See
> [`AGENTS.md`](../AGENTS.md) for the repository map,
> [`docs/account-system-operations.md`](../docs/account-system-operations.md) for
> operations, and
> [`docs/account-system-acceptance.md`](../docs/account-system-acceptance.md) for
> dated, secret-free acceptance evidence.

## Context

ACN FDE remains a complete client-only React 19/Vite application in standalone guest mode. Its optional full-stack path adds a same-origin Cloudflare Worker, Better Auth, CoreEx, D1, and PostgreSQL without making any of them requirements for guest practice. The production deployment keeps the roughly US$5/month Cloudflare posture, uses GitHub-only sign-in, and merges a guest's existing practice state through the ordinary idempotent sync path after sign-in.
...
Everything below exists and is wired into the checked-in production target and Worker configuration ([#35](https://github.com/timbarreto/acn-fde/issues/35)).
...
| `BETTER_AUTH_SECRET` | AppHost .NET user-secrets, key `Parameters:better-auth-secret` | Worker secret; rotating it invalidates every existing session |
...
- **Key rotation is off by default** and lazy — it happens on the next signing operation, never on a schedule, `gracePeriod` defaults to 30 days, and old `jwks` rows are never deleted. This deployment does not configure scheduled rotation; adding it requires a separate rotation and retained-key acceptance change.
...
The view was delivered in two parts. [#76](https://github.com/timbarreto/acn-fde/issues/76) shipped the destination itself: the sync-state panel, the optional GitHub sign-in invitation shown only here, the re-authentication path after a lost session, and strict safe sign-out. [#77](https://github.com/timbarreto/acn-fde/issues/77) adds the client-side export, guest and user reset, and ordered account-deletion controls to that view. For a signed-in user, the sync-state panel also identifies the current GitHub identity by avatar and `@username`; guests and re-authentication states show neither. The browser reads those presentation fields from the Better Auth session and does not add them to practice-state storage.
...
**Deletion** needs explicit endpoints because a merge cannot express removal. Guest reset removes and recreates only `agentic-ready-gh600-v2:guest`; it never enumerates or alters account caches. User reset stops new local changes, waits for any in-flight request to settle, waits for server deletion to be accepted, removes only the cache derived from the authenticated subject, and keeps that identity signed in with an empty in-memory account state.

- *Delete all my practice data* — `DELETE /api/practice-state`, keeping the account. The label must say what it removes: it deletes finished attempts, bookmarks, and latest answers, not merely the statistics a candidate sees ([#51](https://github.com/timbarreto/acn-fde/issues/51)). This matters more than it looks for a practice-exam tool: question selection is driven by prior answers, so starting fresh is a plausible request.
- *Delete my account* — `DELETE /api/practice-state` **first**, then Better Auth's `POST /api/auth/delete-user`. No transaction spans D1 and PostgreSQL, so the order is the safety mechanism: a mid-way failure leaves an account with no data, which is harmless and retryable, rather than an orphaned row nobody can reach. After PostgreSQL accepts deletion, the browser replaces the subject cache with an empty `accountDeletionStage: "identity"` continuation marker before calling Better Auth. A failed or interrupted identity step therefore resumes without restoring or re-deleting practice data; navigation remains pinned to Account, and same-account GitHub re-authentication is available when Better Auth requires a fresh session. Success removes that subject's marker and starts a new empty guest practice state.

**What is stored about a person:** a Better Auth user id, GitHub display name, username, avatar URL, and email (Better Auth's schema requires it `NOT NULL UNIQUE`), plus the GitHub account id. The provider mapping refreshes the username and avatar on each later sign-in so GitHub profile changes reach the Account view. The GitHub **access token is not retained** — it is cleared once sign-in completes, since nothing calls the GitHub API afterwards. State this list in the README.
...
**Closed-world identifiers:** `contracts/question-recognition-manifest.json` is an append-only recognition manifest derived initially from `src/data/questions.json`. It records every historically issued question ID, its selection mode, the option IDs valid for that question, and the correct answer IDs needed to score an active attempt rescued as abandoned during a merge; it does not copy question wording. Removing a question from the live bank never removes its recognition entry. The backend must recognise new IDs before frontend assets can emit them. Unknown question IDs, unknown option IDs, and invalid question/option pairings are rejected. This bounds the merge-only JSON maps, lets the server preserve a displaced attempt without depending on frontend code, and keeps old offline clients valid.
...
- **Secrets:** the helper stores the local GitHub OAuth client ID/secret and Better Auth secret in AppHost .NET user-secrets. The Development AppHost declares those three values as secret parameters and attaches only their named environment variables to Vite. The Vite Cloudflare-plugin configuration then copies only those names into the local Worker's vars. `CLOUDFLARE_INCLUDE_PROCESS_ENV` is deliberately not enabled, so unrelated process values such as `PATH` and `HOME` never enter the Worker's `env`. PostgreSQL uses Aspire-generated local credentials. Cloudflare, production Better Auth, and Neon secrets are not required locally and never enter committed files.
...
- **AppHost configurations** (not "profiles" — Aspire has no such platform feature): Development keeps PostgreSQL/D1 state for iterative work and receives the three named auth parameters; Integration uses isolated temporary storage, test-only auth, and allocated non-OAuth ports; Container uses the same isolation while replacing the API project resource with `AddDockerfile` plus `WithContainerRuntimeArgs("--memory", "1g")`. The Container resource explicitly receives OTLP and service-discovery wiring that project resources obtain automatically.
...
`Acn.Fde.Practice.IntegrationTests` uses `Aspire.Hosting.Testing` as a normal test project that references the AppHost rather than creating a second AppHost. It launches the isolated Integration or Container configuration and calls the Vite/Worker same-origin URL. It waits only on resources with registered checks. A separate Worker test entry wraps Better Auth's `testUtils` session creation; it is referenced only by `wrangler.integration.jsonc`, binds to the isolated stack, exposes no production route, and is excluded from the production entry/config. Those fixtures mint real ES256 JWTs on local D1 without contacting GitHub, but do not create a GitHub provider-account row; real provider linking is covered separately by the Worker callback test and manual OAuth acceptance.
...
- Validation and scoring contract: new append-only `contracts/question-recognition-manifest.json`, initially derived from `src/data/questions.json`, plus tests that keep its question IDs, selection modes, option IDs, and correct answer IDs aligned with the live bank; the backend uses the answer key only to score an active attempt rescued as abandoned during merge
...
- Aspire: `backend/src/Acn.Fde.Practice.AppHost/` defines Development, Integration, and Container resource graphs; `backend/tests/Acn.Fde.Practice.IntegrationTests/` hosts configuration, full-stack, and resilience tests
- Local/CI/deployment: npm/Aspire orchestration scripts, ignored local D1 storage, the non-Playwright CI workflow, secret/environment examples, `setup-github-secrets.sh`, and `scripts/production/` for priming, preparation, deployment, and recovery
- Guidance: `README.md`, `AGENTS.md`, `docs/account-system.md`, `docs/account-system-operations.md`, and `docs/account-system-acceptance.md`
...
- [x] Add the Worker entry point and bindings: serve Vite assets, configure D1 and committed auth migrations, initialize Better Auth with GitHub + JWT/JWKS, and route API traffic to one sleeping `basic` CoreEx Container.
- [x] Scaffold `Acn.Fde.Practice` with CoreEx's PostgreSQL API templates and remove/avoid unused reference-data, messaging, outbox, relay, subscriber, Redis, and domain-layer features.
- [x] Implement the PostgreSQL state schema and migrations, CoreEx contracts/validators/service/repository/controllers, the 512 KiB body gate and closed-world recognition manifest, the merge-on-write transaction, authenticated ownership, JWT validation, health checks, and backend tests; publish OpenAPI and verify the handwritten frontend adapter against the running API.
...
- [x] Add the Better Auth client, thin React persistence hook, GitHub sign-in/out UI, single-flight in-memory token adapter, and typed state client — including first-sync consume ordering, session-loss quarantine, and strict flush-before-sign-out blocking from #56.
- [x] Add the `Account` view as a fifth top-level nav item available to guests and signed-in users, holding sync state, optional GitHub sign-in, re-authentication, and strict safe sign-out; add the always-visible `TopNav` sync indicator with the six states above, and keep it out of `ExamRunner`'s separate header. No modal, banner, or prompt anywhere invites sign-in (#50, #76).
- [x] Add the `Account` data controls — the client-side JSON export, self-service reset, and ordered account deletion — to that same view (#77).
- [x] Add the Aspire AppHost and its Development, Integration, and Container configurations: provision PostgreSQL, gate API startup on CoreEx migrations, gate Vite/workerd startup on local D1 migrations and API readiness, pass only named local auth secrets, publish dashboard links/health, and provide `dev:full` plus narrowly scoped local reset instructions.
- [x] Add the Aspire full-stack test project and test-only Better Auth entry/config; exercise the same-origin stack with isolated stores and no GitHub/network dependency, then add standard CI for TypeScript/.NET/AppHost/container validation.
- [x] Add secret-safe production configuration, rollback priming, the one-time Worker-secret bootstrap helper, additive migration gates, and the operator-run deployment script: preflight a clean current `main`; apply and ledger-check PostgreSQL then D1 migrations; re-check the active Worker version; deploy the singleton with an immediate Container rollout; run the one-minute anonymous health gate; and emit the secret-free release summary. Implement stop/resume, partial-deploy recovery, and failed-health-reporting paths exactly as specified above.
- [x] Update README, candidate guidance, operator guidance, and agent guidance with the implemented architecture, setup, stored identity data, 6-hour recovery window, client-cache durability, cost assumptions/limits, troubleshooting, and secret-free acceptance record. Sign-in remains **optional** and standalone guest practice remains offline-first and complete (#50, #81).

## Verification

This section records the supported verification interfaces. Dated results and the mapping from each interface to accepted behavior live in [`docs/account-system-acceptance.md`](../docs/account-system-acceptance.md).
...
- `dotnet test backend/Acn.Fde.Practice.slnx`: CoreEx validators/services/repositories/controllers plus merge idempotency and conflict scenarios; request-validation boundaries and Problem Details codes; transient retention and future-clock handling; valid, expired, wrong-issuer/audience ES256 JWTs and JWKS retrieval; and authorization filters.
...
Run `npm run test:full` to start the AppHost's Integration configuration with disposable PostgreSQL and local D1 stores, wait for migrations and registered health checks, and test through the Vite/Worker same-origin endpoint. The suite fetches the SPA and local health routes, verifies anonymous practice requests return `401`, creates two independent test identities through the integration-only auth entry, saves and loads different envelopes, proves reset of one subject leaves the other unchanged, deletes the second subject's practice state before its Better Auth identity, and confirms the deleted session is gone.

The broader merge, validation, first-sync, offline, concurrent-edit, rollback, and cache-isolation scenarios run at their focused frontend and backend interfaces under `npm run test` and `npm run test:backend`. Restart and production-image behavior runs only under `npm run test:resilience`, as described below. The isolated full-stack test deletes its temporary stores after the run and emits Aspire resource logs on failure. It does not contact GitHub, Cloudflare, or Neon.
...
With local GitHub OAuth user-secrets configured, run `npm run dev:full`, open the frontend from the Aspire dashboard, and verify real GitHub sign-in/callback, a guest's first sync after signing in, reload, sign-out/cache isolation, offline guest practice, API/database outage messaging, OpenAPI, health, logs, traces, and clean recovery. Use the Container configuration once to check the production image locally. Record the result without identity data in [`docs/account-system-acceptance.md`](../docs/account-system-acceptance.md).
